...
@pytest.mark.parametrize(
    "timestamp, expected",
    [
        ("2023-01-01Z", datetime.datetime(2023, 1, 1)),
...
def test_error_detection_all_ok(
...
@pytest.mark.parametrize(
    "start, end, attendances, employees, error_count, reason", [HOURS_6_SCENARIO, HOURS_9_SCENARIO, HOURS_10_SCENARIO]
)
...
PRECONDITION_SCENARIO = (datetime.datetime(2023, 1, 1), datetime.datetime(2023, 3, 15), precondition_, EMPLOYEES, 1)


@pytest.mark.parametrize("start, end, attendances, employees, error_count", [PRECONDITION_SCENARIO])
def test_precondition_detection(
    start: datetime.datetime,
    end: datetime.datetime,
    attendances: factorial.LIST_JSON_RESPONSE,
    employees: factorial.LIST_JSON_RESPONSE,
